...
const { updateUserBadgeIfNeeded } = require('../services/badgeService');
...
const googleLogin = async (req, res, next) => {
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.log('[GoogleOAuth] Verifying token with audience:', clientId);
    }
    const ticket = await client.verifyIdToken({ idToken: credential, audience: clientId });
    const payload = ticket.getPayload();
    if (process.env.NODE_ENV !== 'production') {
      console.log('[GoogleOAuth] Token verified for:', payload?.email || 'unknown');
    }
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.error('[GoogleOAuth] Token verification failed:', error?.message || error);
    }
    next(error);
  }
};
